        ...
        while True:
            ...
            if sequence_status['curr'] == 0:
                            ...
                            while True:
                                ...
                                if num_tokens > self.max_num_tokens_per_sample:
                                    if len(video_buffer) < self.max_buffer_size:
                                        ...
                                        print(f"Added sample with length {num_tokens} to video_buffer (size: {len(video_buffer)})")
                                    else:
                ...
                if sequence_status['curr'] >= self.expected_num_tokens:
                    ...
                    print(f"Yielding {len(sequence_status['sample_lens'])} 3D data with length {sum(sequence_status['sample_lens'])}, length of each sample: {sequence_status['sample_lens']}")
                    yield data
                    sequence_status = self.set_sequence_status()
                    batch_data_indexes = []
