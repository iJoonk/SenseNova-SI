...
def concat_pad_data_collator(features, max_item_length=None, pad_id=0, len2weight=None, loss_reduction_all_gather=False):
    ...
    for idx in range(len(features)):
        ...
        # Handle loss_weight from packed datasets
    ...
    # Compute loss_weight for non-packed datasets if len2weight is provided
    if len2weight is not None and 'loss_weight' not in batch:
        # For non-packed datasets, compute weight for each sample in the batch
        batch_size = batch['labels'].shape[0]
        loss_weight = torch.zeros_like(batch['labels'], dtype=torch.float32)
        
        for i in range(batch_size):
            # Count effective tokens for this sample
            sample_labels = batch['labels'][i]
            num_effective_tokens = (sample_labels != IGNORE_INDEX).sum().item()
            
            # Compute weight for this sample
            weight_value = len2weight(num_effective_tokens)
            
            # Assign weight to all positions in this sample
            loss_weight[i] = weight_value
        
        batch['loss_weight'] = loss_weight
    
    # Convert loss_weight to list if it exists (to prevent bf16 conversion issues)
    if 'loss_weight' in batch:
        batch['loss_weight'] = torch.where(batch['labels'] == IGNORE_INDEX, 0, batch['loss_weight']).tolist()
        # Add loss_reduction_all_gather flag when using weighted loss
