...
from internvl.train.dataset import (ConcatDataset,
                                    ...
                                    preprocess_phi3,
                                    read_frames_gif, read_frames_decord)
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            ...
            try:
                # Skip empty lines
                while not self.raw_data[i].strip():
                ...
                if not isinstance(e, (UnidentifiedImageError, FileNotFoundError)):
                    traceback.print_exc()
                # Try to print debug info, but handle empty lines gracefully
                try:
...
def main():
    ...
    if data_args.use_packed_ds:
        ...
        # Use weighted loss reduction for non-packed datasets as well
        collator = partial(
